...
# (write an expression)
np.divide(part_num[mask].size, part_num.size)
...
# (write an expression)
( length - length.min()) / ( (length.max() - length.min()) )
...
# (assignment to parts)
parts = np.concatenate( (width, length), axis = 0)
...
# (assignment to parts)
parts = parts.reshape(9,-1, order='F')
...
# assume array 'parts' is defined
# (write an expression)
np.sqrt(( np.subtract(parts[0,0], parts[1,0]) ** 2) + ( np.subtract(parts[0,1], parts[1,1]) ** 2))
...
def distance(width, length, parts):
    # Create an empty array to return -> Change to an np array before returning
    output = []

    # Find the point of each parts array
    for p in range(0, len(parts)):

        # Calculate the distance between parts array point and input point
        dist = np.sqrt(( np.subtract(parts[p,0], width) ** 2) + ( np.subtract(parts[p,1], length) ** 2))

        # Add the distance calculated to the output array
        output.append(dist)

    # Return np array of output
...
def most_similar_part(width, length, parts):
    ...
    # Create an array that we will return
    output = np.array([])

    # Calculate distances of given part to each part using distance function
    dist = distance(width, length, parts) # type: np array

    # minimum distance to the unknown part, or within 0.01 of that distance.
    min_val = dist.min() # get min val

    # """part_num[np.where(dist == np.amin(dist))] or"""
    output = part_num[np.where(dist-min_val < 0.01)]
