...
num_measles = np.bincount(has_measles)[1]
...
# for i in range(0, len(has_measles)):
#     result = np.array([])
#     if has_measles[i] == True:
#         result = np.random.choice([True, False], 1, p=[.90, .10])
#         tests_positive[i] = (result[0])
#     elif has_measles[i] == False:
#         result = np.random.choice([True, False], 1, p=[0.05, 0.95])
#         tests_positive[i] = (result[0])
...
# sum of has measles / sum of pos test
...
#@ 7
# Using the has_measles and tests_positive arrays, compute the probability 
# that a person does not have measles given negative test results.
# (expression)
...
# doesn't have measles / negative test
np.sum(~tests_positive[~has_measles])/np.sum(~tests_positive)
...
# Take everything from above and put it in a function basically 
def sim_p_measles_if_pos(p_measles, p_pos_if_measles, p_pos_if_okay):
    # First Step: Fill out the arrays has_measles and num_measles
    has_measles = np.random.choice([True, False], n, p=[p_measles, 1-p_measles])
    num_measles = np.sum(has_measles)
    tests_positive = np.full(shape=n,fill_value=False)
    
    # Update the positive test results 
    tests_positive[has_measles] = np.random.choice([True,False],size=num_measles,p=[p_pos_if_measles, 1-p_pos_if_measles])
    tests_positive[~has_measles] = np.random.choice([True,False],size=n-num_measles,p=[p_pos_if_okay, 1-p_pos_if_okay])
...
# prob of having measleas if positive * gen prob of measles / 
# prob of having measleas if positive * gen prob of measles + the probability of a positive test given no measles * Probab of not having measles
